...
const { sources = {}, ...full } = JSON.parse(await readFile('scripts/payload.json', 'utf8'))
...
<script>const payload=${JSON.stringify(payload)};const sources=${JSON.stringify(sources)};let s={};const reply=m=>setTimeout(()=>window.postMessage(m,'*'),0);window.acquireVsCodeApi=()=>({postMessage:m=>{if(m.type==='ready')reply({type:'review',payload});if(m.type==='loadSource')reply({type:'source',blob:m.blob,text:sources[m.blob]??''})},setState:x=>{s=x},getState:()=>s})</script>
